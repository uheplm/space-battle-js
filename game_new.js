...
class Player{
    ...
    draw() { 
        ...
        for (var i = 0; i < this.abilityAmmo; i++) {
            session.ctx.fillRect(50+15*i,70,10,20); 
        }
        ...
        session.ctx.fillText(this.abilityAmmo, 40, 70);
        return this;
    }
}
...
function playerMove(e){
    ...
    if(e.clientX > 0 && e.clientX < session.canvas.width) session.player.x = e.clientX - 25;
}
...
function updateEnemies(){
    ...
    for (var i = Enemies.length - 1; i >= 0; i--){
        if(Enemies[i].x > 0){
            ...
            session.entities.enemies.splice(i,1);
            session.player.hurt();
        }
    }
...
function playerShoot() {
    if(session.player.ammo > 0){
        ...
        newBullet = new Bullet(session.player.x+session.player.width,session.player.y+session.player.height/2);
        newBullet.setVelocity(10,0);
        newBullet.color = '#0f0';
        newBullet.setSize(10);
        session.player.ammo--;
        session.entities.bullets.push(newBullet);
    }else{
        playerReload()
    }
}
...
function increaseDifficulty() {
    session.enemiesHP++;
    session.spawnSpeed -= 100;
    session.enemiesFireSPD -= 100;
